...
COPY users(id, fio, telephone_number, city, user_login, user_password, status, user_description) 
FROM '/docker-entrypoint-initdb.d/users.csv' 
DELIMITER ',' 
CSV HEADER
ENCODING 'UTF8';

COPY pet(id, vet_passport_id, seller_id, pet_name, species, pet_age, color, pet_gender, 
          breed, pedigree, good_with_children, good_with_animals, pet_description, 
          is_active, price) 
FROM '/docker-entrypoint-initdb.d/pet.csv' 
DELIMITER ',' 
CSV HEADER
ENCODING 'UTF8';
